...
/**
 * ZooKeeper 客户端主类。使用ZooKeeper 服务的应用需要实例化一个ZooKeeper 类。
 * 该类的方法都是线程安全的，除非特别指出的。
 * <p>当建立起和服务器之间的连接，对该客户端设置一个会话ID。客户端会定期发送心跳来保持会话有效
 * <p>只要会话ID 有效，应用就可用客户端调用ZooKeeper 的API
 * <p>如果客户端发送心跳时间超过超时时间，服务器会过期会话。此时应用需要创建新的客户端对象
 * <p>如果ZooKeeper 服务端未响应，客户端会在会话过期前自动重连其他服务器。如果成功，该客户可继续使用
 * <p>ZooKeeper 的API 分为同步和异步两部分，同步方法会阻塞到服务端响应。异步方法将请求放入队列并立即返回
 * <p>watch 事件对统一客户端只会触发一次
 * <p>客户端需要实现{@link Watcher} 接口的类来处理传递到客户端的事件
 * 当客户端失去当前连接并重连服务器时，所有的watch 被认为触发了，但未投递到的事件丢失了。
 * 为模拟该情况，客户端会生成一个特殊事件来告诉事件处理器连接丢失。该事件的EventType 是None
 * KeeperState 是Disconnected
 *
...
public class ZooKeeper implements AutoCloseable {
    ...
    static class ZKWatchManager implements ClientWatchManager {
       ...
       */
        private boolean contains(String path, Watcher watcherObj, Map<String, Set<Watcher>> pathVsWatchers) {
            ...
            if (pathVsWatchers == null || pathVsWatchers.size() == 0) {
                ...
                if (watchers == null) {
                    ...
                    // 传入的watcherObj 为null 且集合中存在元素则为存在
        ...
        /**
         * 校验给定znode 路径是否包含给定watcher 和watcherType
         *
        ...
        /**
         * 删除给定路径下的给定watcher，当watcher 为{@code null} 时删除给定路径下的所有wathcer
         *
         * @param pathVsWatcher
         * @param watcher 需要删除的watcher
         * @param path
         * @param local is local operation
         * @param rc returnCode {@link Code}
         * @param removedWatchers 被删除的watcher
         * @return
        ...
        protected boolean removeWatches(
            ...
            // 当local 标志位true 时，删除watcher 与返回值无关
    ...
    /**
     * 用给定的路径创建节点并返回节点状态。节点数据为给定数据，节点ACL 为给定ACL
     * <p>标志指定创建的节点是否是临时的
     * <p>临时节点会在创建节点的会话过期时被ZooKeeper 删除
     * <p>标志通用指定是否创建顺序节点。顺序节点的实际路径会加后缀，固定10位数字，用0填充
     * <p>如果ZooKeeper 中有同样的路径，会抛出KeeperException.NodeExists
     * <p>如果父节点不存在会抛出KeeperException.NoNode
     * <p>临时节点不能有子节点。如果给定节点的父节点是临时节点，则抛出
     * KeeperException.NoChildrenForEphemerals
     * <p>如果操作成功会触发所有该节点的exist getDate API 调用
     * <p>如果节点创建成功，ZooKeeper 服务器会触发该路径上的exist，watch 在父节点的会收到
     * getChildren 调用
     * <p>允许最大数据数组为1MB，超出会抛出异常
     *
